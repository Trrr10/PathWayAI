 ...
 *
 * Flow:
 *  1. Landing — employer listings + "Build Resume" CTA
 *  2. Resume Wizard — multi-step info collection (7 steps)
 *  3. AI Generation — Llama builds the resume
 *  4. Resume Viewer — full resume with inline editing + version history
 */
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .slide-l   { animation: slide-l  0.4s cubic-bezier(0.16,1,0.3,1) both; }
  ...
  .resume-divider { height: 1.5px; margin-bottom: 12px; }
  ...
  .editable:hover::after {
    ...
    background: rgba(56,189,248,0.1); padding: 2px 6px; border-radius: 4px;
    font-family: 'DM Sans', sans-serif; pointer-events: none;
    white-space: nowrap;
  }
  .editable.edited::before {
    content: '●';
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: "8px 10px", borderRadius: 12, border: `1.5px solid ${dark ? "rgba(59,130,246,0.2)" : "rgba(147,197,253,0.4)"}`, background: dark ? "rgba(15,30,55,0.8)" : "rgba(248,250,252,0.9)", minHeight: 44, alignItems: "center" }}>
      {tags.map(t => (
        <span key={t} className="tag">
          {t}
          <span className="tag-x" onClick={() => remove(t)}>×</span>
        </span>
      ))}
      <input
        value={input} placeholder={tags.length === 0 ? placeholder : "Add more…"}
...
function EditableField({ value, onChange, multiline, style, className, isEdited }) {
  ...
  if (editing) {
    const shared = {
      ...
      autoFocus: true,
      style: { ...style, background: "rgba(56,189,248,0.08)", border: "2px solid #38BDF8", borderRadius: 6, padding: "2px 6px", outline: "none", fontFamily: "inherit", fontSize: "inherit", fontWeight: "inherit", color: "inherit", width: "100%", resize: multiline ? "vertical" : "none" },
    };
  ...
  return (
    <span
      className={`editable ${isEdited ? "edited" : ""} ${className || ""}`}
      style={{ ...style, display: "block", padding: "2px 4px", cursor: "text", position: "relative" }}
    ...
    <div className="resume-wrap" style={{ background: dark ? "#0D1B2E" : "#FFFFFF", color: t, padding: "40px 44px", borderRadius: 16, maxWidth: 760, margin: "0 auto", boxShadow: dark ? "0 20px 60px rgba(0,0,0,0.6)" : "0 20px 60px rgba(37,99,235,0.12)", position: "relative" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 28, paddingBottom: 24, borderBottom: `2px solid ${divColor}` }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: 16, marginTop: 10, fontSize: 12, color: m }}>
          ...
          <span>·</span>
          {field("phone", resume.phone, { style: { fontSize: 12, color: m } })}
          <span>·</span>
          {field("location", resume.location, { style: { fontSize: 12, color: m } })}
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
            {resume.skills.map((s, i) => (
              <span key={i} style={{ padding: "3px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600, background: dark ? "rgba(29,78,216,0.2)" : "rgba(219,234,254,0.8)", border: `1px solid ${dark ? "rgba(59,130,246,0.3)" : "rgba(147,197,253,0.5)"}`, color: dark ? "#93C5FD" : "#1D4ED8", fontFamily: "'DM Sans',sans-serif" }}>
                {s}
              </span>
            ))}
          </div>
...
export default function Employers() {
  ...
  const [screen, setScreen]   = useState("employers"); // employers | wizard | generating | resume
  ...
  const [versions, setVersions]       = useState([]); // [{label, resume, editedFields}]
  ...
  const generateResume = async () => {
    ...
    const prompt = `You are an expert resume writer. Create a complete, professional resume in JSON format based on this student's information.
...
- School: ${form.school} (${form.schoolYear}, ${form.schoolScore})
- College/Current: ${form.college} - ${form.degree} (${form.collegeYear}, ${form.collegeScore})
- Technical Skills: ${form.techSkills.join(", ")}
- Soft Skills: ${form.softSkills.join(", ")}
- Projects: ${form.projects.map(p => `${p.name} (${p.tech}): ${p.description}`).join(" | ")}
- Experience: ${form.experience.map(e => `${e.role} at ${e.company} (${e.duration}): ${e.description}`).join(" | ")}
- Achievements: ${form.achievements.join(", ")}
- Target Role: ${form.targetRole}
- Target Company: ${form.targetCompany}
- Tone: ${form.targetTone}

Return ONLY this JSON structure, no other text:
{
  "name": "full name",
  "tagline": "compelling one-line professional tagline targeting the role",
  "email": "email",
  "phone": "phone",
  "location": "location",
  "linkedin": "linkedin url",
  "summary": "3-4 sentence professional summary highlighting key strengths, skills, and career objective tailored to ${form.targetRole}",
  "education": [
    {"school": "school name", "degree": "board/stream", "year": "year/batch", "score": "score/percentage"},
    {"school": "college name", "degree": "degree program", "year": "year/batch", "score": "score/cgpa"}
  ],
  "skills": ["skill1", "skill2", ...all tech and soft skills combined, max 16],
  "projects": [
    {"name": "project name", "tech": "tech stack", "description": "2-3 sentence impact-focused description with metrics if possible"}
  ],
  "experience": [
    {"role": "role title", "company": "company", "duration": "duration", "description": "2-3 bullet points of achievements"}
  ],
  "achievements": ["achievement 1", "achievement 2", ...]
}

Make the summary and project descriptions compelling and ATS-optimized for ${form.targetRole} at ${form.targetCompany || "top companies"}.
Use strong action verbs. Quantify impact where possible. Keep it concise but impressive.`;
    ...
    try {
      ...
      if (!match) throw new Error("AI didn't return valid JSON. Try again.");
      const parsed = JSON.parse(match[0]);

      const initial = { label: "AI Draft v1", resume: parsed, editedFields: new Set() };
      setVersions([initial]);
      setActiveVersion(0);
      setResume(parsed);
  ...
  const handleEdit = useCallback((key, value) => {
    ...
    setVersions(prev => {
      ...
      // Only add snapshot if something actually changed
      if (!editedFields.has(key) || current.resume[key] !== value) {
        newVersions.push({
          label: `Edit ${editedFields.size + 1}`,
          resume: { ...resume },
    ...
    setResume(prev => {
      // Handle nested keys like edu_0_school
  ...
  const renderWizStep = () => {
    const inputStyle = { className: "field" };

    switch (wizStep) {
      case 0: return (
        <div className="slide-r" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><span className="lbl">Full Name *</span><input className="field" value={form.name} onChange={e => setF("name", e.target.value)} placeholder="Kavya Sharma" /></div>
            <div><span className="lbl">Email *</span><input className="field" type="email" value={form.email} onChange={e => setF("email", e.target.value)} placeholder="kavya@email.com" /></div>
            ...
            <div><span className="lbl">City / Location</span><input className="field" value={form.location} onChange={e => setF("location", e.target.value)} placeholder="Pune, Maharashtra" /></div>
            <div><span className="lbl">LinkedIn URL</span><input className="field" value={form.linkedin} onChange={e => setF("linkedin", e.target.value)} placeholder="linkedin.com/in/kavya" /></div>
            <div><span className="lbl">GitHub (optional)</span><input className="field" value={form.github} onChange={e => setF("github", e.target.value)} placeholder="github.com/kavya" /></div>
          </div>
        </div>
      );
        ...
        <div className="slide-r" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              <div style={{ gridColumn: "span 3" }}><span className="lbl">School Name *</span><input className="field" value={form.school} onChange={e => setF("school", e.target.value)} placeholder="Delhi Public School, Pune" /></div>
              ...
              <div><span className="lbl">Score / %</span><input className="field" value={form.schoolScore} onChange={e => setF("schoolScore", e.target.value)} placeholder="91.4%" /></div>
            </div>
          </div>
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, color: t, marginBottom: 12 }}>🎓 College / University (if applicable)</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div style={{ gridColumn: "span 2" }}><span className="lbl">College Name</span><input className="field" value={form.college} onChange={e => setF("college", e.target.value)} placeholder="COEP Technological University" /></div>
      ...
      case 2: return (
        <div className="slide-r" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            ...
            <TagInput tags={form.techSkills} onChange={v => setF("techSkills", v)} placeholder="Python, React, SQL, Machine Learning…" dark={dark} />
            <p style={{ fontSize: 11, color: m, marginTop: 6 }}>💡 Add at least 4–6 skills. Be specific — "React.js" beats "Web Dev"</p>
          </div>
          <div>
            <span className="lbl">Soft Skills</span>
            <TagInput tags={form.softSkills} onChange={v => setF("softSkills", v)} placeholder="Communication, Leadership, Problem Solving…" dark={dark} />
          </div>
          <div style={{ padding: "12px 16px", borderRadius: 12, background: dark ? "rgba(56,189,248,0.08)" : "rgba(219,234,254,0.5)", border: `1px solid ${dark ? "rgba(56,189,248,0.2)" : "rgba(147,197,253,0.4)"}` }}>
            <p style={{ fontSize: 12, color: "#38BDF8", fontWeight: 700, marginBottom: 4 }}>🎯 Your PathwayAI Skills</p>
            <p style={{ fontSize: 12, color: m }}>Based on your credentials and quiz history, we've detected these skills. Click to add:</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
              {["Mathematics", "Python Basics", "Problem Solving", "Data Analysis"].map(s => (
                <button key={s} onClick={() => !form.techSkills.includes(s) && setF("techSkills", [...form.techSkills, s])}
                  style={{ padding: "4px 12px", borderRadius: 20, border: "1px solid rgba(56,189,248,0.4)", background: "transparent", color: "#38BDF8", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
                  + {s}
                </button>
              ))}
            </div>
      ...
      case 3: return (
        <div className="slide-r" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <p style={{ fontSize: 13, color: m }}>Add your best 1–3 projects. These matter most to employers!</p>
          {form.projects.map((p, i) => (
            <div key={i} style={{ padding: 16, borderRadius: 14, border: `1px solid ${dark ? "rgba(59,130,246,0.15)" : "rgba(147,197,253,0.3)"}`, background: dark ? "rgba(15,30,55,0.5)" : "rgba(248,250,252,0.7)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div><span className="lbl">Project Name</span><input className="field" value={p.name} onChange={e => { const ps = [...form.projects]; ps[i].name = e.target.value; setF("projects", ps); }} placeholder="PathwayAI Tutor" /></div>
                  <div><span className="lbl">Tech Stack</span><input className="field" value={p.tech} onChange={e => { const ps = [...form.projects]; ps[i].tech = e.target.value; setF("projects", ps); }} placeholder="React, Node.js, Ollama" /></div>
                </div>
                <div><span className="lbl">What it does & your role</span><textarea className="field" rows={2} value={p.description} onChange={e => { const ps = [...form.projects]; ps[i].description = e.target.value; setF("projects", ps); }} placeholder="Built an AI-powered tutoring app that…" /></div>
              </div>
            </div>
          ))}
          {form.projects.length < 3 && (
            <button onClick={() => setF("projects", [...form.projects, { name: "", tech: "", description: "" }])} className="btn btn-ghost" style={{ alignSelf: "flex-start" }}>
              + Add Another Project
            </button>
          )}
        </div>
      ...
      case 4: return (
        <div className="slide-r" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <p style={{ fontSize: 13, color: m }}>Add internships, part-time jobs, or volunteer experience. Skip if none.</p>
          {form.experience.map((ex, i) => (
            <div key={i} style={{ padding: 16, borderRadius: 14, border: `1px solid ${dark ? "rgba(59,130,246,0.15)" : "rgba(147,197,253,0.3)"}`, background: dark ? "rgba(15,30,55,0.5)" : "rgba(248,250,252,0.7)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
                <div><span className="lbl">Role / Title</span><input className="field" value={ex.role} onChange={e => { const es = [...form.experience]; es[i].role = e.target.value; setF("experience", es); }} placeholder="Software Intern" /></div>
                <div><span className="lbl">Company</span><input className="field" value={ex.company} onChange={e => { const es = [...form.experience]; es[i].company = e.target.value; setF("experience", es); }} placeholder="TCS, Startup, NGO…" /></div>
                <div><span className="lbl">Duration</span><input className="field" value={ex.duration} onChange={e => { const es = [...form.experience]; es[i].duration = e.target.value; setF("experience", es); }} placeholder="Jun–Aug 2024 · 3 months" /></div>
              </div>
              <div><span className="lbl">Key Responsibilities / Achievements</span><textarea className="field" rows={2} value={ex.description} onChange={e => { const es = [...form.experience]; es[i].description = e.target.value; setF("experience", es); }} placeholder="What did you build / achieve? Any numbers?" /></div>
            </div>
          ))}
          {form.experience.length < 3 && (
            <button onClick={() => setF("experience", [...form.experience, { role: "", company: "", duration: "", description: "" }])} className="btn btn-ghost" style={{ alignSelf: "flex-start" }}>
              + Add Experience
            </button>
          )}
        </div>
      ...
      case 5: return (
        <div className="slide-r" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <p style={{ fontSize: 13, color: m }}>Awards, certifications, competitions, scholarships — anything impressive!</p>
          {form.achievements.map((a, i) => (
            <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <input className="field" style={{ flex: 1 }} value={a} onChange={e => { const ac = [...form.achievements]; ac[i] = e.target.value; setF("achievements", ac); }} placeholder={`e.g. ${["State Math Olympiad Winner 2023", "Google DSC Lead", "National Coding Hackathon — 2nd Place", "Infosys Certified Python Developer"][i % 4]}`} />
          ...
          {form.achievements.length < 6 && (
            <button onClick={() => setF("achievements", [...form.achievements, ""])} className="btn btn-ghost" style={{ alignSelf: "flex-start" }}>
              + Add Achievement
            </button>
          )}
          <div style={{ padding: "12px 16px", borderRadius: 12, background: dark ? "rgba(245,158,11,0.08)" : "rgba(254,243,199,0.7)", border: `1px solid ${dark ? "rgba(245,158,11,0.2)" : "rgba(251,191,36,0.4)"}` }}>
            <p style={{ fontSize: 12, color: "#F59E0B", fontWeight: 700 }}>🏅 From your PathwayAI credentials:</p>
            <p style={{ fontSize: 12, color: m, marginTop: 4 }}>Your verified badges will be auto-included: Mathematics Silver Badge, Coding Bronze Badge, Science Bronze Badge</p>
          </div>
        </div>
      );
      ...
      case 6: return (
        <div className="slide-r" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <p style={{ fontSize: 13, color: m }}>Tell AI exactly what role you're targeting so it can tailor every word of your resume.</p>
          <div><span className="lbl">Target Job Role *</span><input className="field" value={form.targetRole} onChange={e => setF("targetRole", e.target.value)} placeholder="Software Engineer Intern, Data Analyst, Product Manager…" /></div>
          <div><span className="lbl">Target Company (optional)</span><input className="field" value={form.targetCompany} onChange={e => setF("targetCompany", e.target.value)} placeholder="TCS, Flipkart, ISRO, Google…" /></div>
          <div>
            <span className="lbl">Resume Tone</span>
            <div style={{ display: "flex", gap: 8 }}>
              {[
                { id: "professional", label: "Professional", desc: "Corporate & formal" },
                { id: "dynamic",      label: "Dynamic",      desc: "Bold & energetic" },
                { id: "academic",     label: "Academic",     desc: "Research-focused" },
              ].map(tone => (
                <button key={tone.id} onClick={() => setF("targetTone", tone.id)}
                  style={{ flex: 1, padding: "10px 8px", borderRadius: 12, border: `2px solid ${form.targetTone === tone.id ? "#38BDF8" : (dark ? "rgba(59,130,246,0.15)" : "rgba(147,197,253,0.3)")}`, background: form.targetTone === tone.id ? (dark ? "rgba(56,189,248,0.12)" : "rgba(219,234,254,0.6)") : "transparent", color: form.targetTone === tone.id ? "#38BDF8" : m, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all 0.2s" }}>
          ...
          {selectedEmployer && (
            <div style={{ padding: "12px 16px", borderRadius: 12, background: dark ? "rgba(56,189,248,0.08)" : "rgba(219,234,254,0.5)", border: `1px solid ${dark ? "rgba(56,189,248,0.2)" : "rgba(147,197,253,0.4)"}` }}>
              ...
              <p style={{ fontSize: 12, color: m }}>AI will tailor your resume for: {selectedEmployer.skills.join(", ")}</p>
            </div>
          )}
        </div>
      );
    ...
    <>
      ...
      <div className={`emp-app ${theme}`}>
        <div className="mesh-fixed">
          ...
          <div className="orb" style={{ width: 350, height: 350, bottom: "5%", left: "-5%", animationDelay: "5s", background: dark ? "radial-gradient(circle,rgba(129,140,248,0.06) 0%,transparent 70%)" : "radial-gradient(circle,rgba(129,140,248,0.09) 0%,transparent 70%)" }} />
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 960, margin: "0 auto", padding: "32px 20px" }}>
          ...
          <div className="fade-up" style={{ textAlign: "center", marginBottom: 36 }}>
            ...
            <p style={{ fontSize: 15, color: m, maxWidth: 480, margin: "0 auto 24px" }}>
              Browse top companies hiring from PathwayAI. Select one to tailor your AI resume specifically for them.
            </p>
            <button className="btn btn-amber" style={{ fontSize: 15, padding: "14px 36px" }} onClick={() => setScreen("wizard")}>
              ✨ Generate My AI Resume with Llama
            </button>
          </div>
              ...
              <div key={emp.id} className={`emp-card fade-up d${(i % 6) + 1}`}>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {emp.roles.map(r => <span key={r} className="pill">{r}</span>)}
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 16, padding: "10px 12px", borderRadius: 10, background: dark ? "rgba(15,30,55,0.6)" : "rgba(241,245,249,0.8)" }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                    {emp.skills.map(s => <span key={s} style={{ fontSize: 11, color: m }}>{s}</span>).reduce((acc, el, i) => i === 0 ? [el] : [...acc, <span key={`dot-${i}`} style={{ color: m, fontSize: 11 }}>·</span>, el], [])}
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <div style={{ padding: "10px 14px", borderRadius: 12, background: dark ? "rgba(15,30,55,0.7)" : "rgba(241,245,249,0.8)", border: `1px solid ${dark ? "rgba(59,130,246,0.15)" : "rgba(147,197,253,0.3)"}`, fontSize: 13, fontWeight: 700, color: "#38BDF8" }}>
                    {emp.stipend}
                  </div>
                </div>
              </div>
  ...
  if (screen === "wizard") return (
    <>
      ...
      <div className={`emp-app ${theme}`}>
        <div className="mesh-fixed">
          <div style={{ position: "absolute", inset: 0, background: dark ? "radial-gradient(ellipse at 40% 30%,#0d2744 0%,#070E1C 65%)" : "radial-gradient(ellipse at 40% 30%,#dbeafe 0%,#F0F6FF 65%)" }} />
        </div>
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 50, background: dark ? "rgba(7,14,28,0.9)" : "rgba(240,246,255,0.9)", backdropFilter: "blur(20px)", borderBottom: `1px solid ${dark ? "rgba(56,189,248,0.1)" : "rgba(147,197,253,0.4)"}`, padding: "14px 24px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1, fontFamily: "'Syne',sans-serif", fontSize: 16, fontWeight: 800, color: t }}>
            ✨ AI Resume Builder
            {selectedEmployer && <span style={{ fontSize: 13, fontWeight: 400, color: m, marginLeft: 8 }}>· Optimising for {selectedEmployer.name}</span>}
          </div>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 680, margin: "0 auto", padding: "32px 20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 0, marginBottom: 36 }}>
            {STEPS.map((s, i) => (
              <>
                <div key={s.id} className="step-dot" style={{ background: i < wizStep ? "#22C55E" : i === wizStep ? "#38BDF8" : (dark ? "rgba(59,130,246,0.12)" : "rgba(147,197,253,0.2)"), color: i <= wizStep ? "white" : m, cursor: i < wizStep ? "pointer" : "default", boxShadow: i === wizStep ? "0 0 0 4px rgba(56,189,248,0.2)" : "none" }} onClick={() => i < wizStep && setWizStep(i)} title={s.title}>
                  {i < wizStep ? "✓" : s.icon}
                </div>
                {i < STEPS.length - 1 && <div key={`line-${i}`} className="step-line" style={{ background: i < wizStep ? "#22C55E" : (dark ? "rgba(59,130,246,0.1)" : "rgba(147,197,253,0.3)") }} />}
              </>
            ))}
          </div>
          ...
          <div style={{ ...cardStyle, padding: 28, marginBottom: 20 }}>
            <div style={{ marginBottom: 22 }}>
              <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 20, fontWeight: 800, color: t, marginBottom: 4 }}>
                {STEPS[wizStep].icon} {STEPS[wizStep].title}
              </h2>
          ...
          {genError && (
            <div style={{ padding: "12px 18px", borderRadius: 12, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", color: "#F87171", fontSize: 13, marginBottom: 16 }}>
              ⚠ {genError}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "space-between" }}>
            <button className="btn btn-ghost" onClick={() => wizStep > 0 ? setWizStep(w => w - 1) : setScreen("employers")}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {wizStep < STEPS.length - 1 ? (
                <>
                  ...
                  <button className="btn btn-primary" disabled={!canNext()} onClick={() => setWizStep(w => w + 1)}>
                    Next → {STEPS[wizStep + 1]?.icon}
                  </button>
                </>
              ) : (
                <button className="btn btn-amber" style={{ fontSize: 15, padding: "13px 32px" }} onClick={generateResume} disabled={!form.name || !form.email || !form.targetRole}>
    ...
    <>
      ...
      <div className={`emp-app ${theme}`} style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
        <div style={{ textAlign: "center", padding: 40 }}>
          <div style={{ width: 80, height: 80, borderRadius: "50%", background: "linear-gradient(135deg,rgba(56,189,248,0.2),rgba(129,140,248,0.2))", border: "2px solid rgba(56,189,248,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, margin: "0 auto 24px", animation: "pulse 2s infinite" }}>🤖</div>
          <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 24, fontWeight: 800, color: t, marginBottom: 8 }}>Llama is Building Your Resume…</h2>
          <p style={{ color: m, fontSize: 14, marginBottom: 20 }}>Analysing your skills, crafting compelling descriptions, optimising for ATS…</p>
          <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
            {[0, 1, 2].map(i => (
              <div key={i} style={{ width: 10, height: 10, borderRadius: "50%", background: "#38BDF8", animation: `pulse 1.2s ${i * 0.2}s ease-in-out infinite` }} />
            ))}
          </div>
          <p style={{ fontSize: 12, color: m, fontStyle: "italic", marginTop: 20 }}>This takes 20–40 seconds…</p>
        </div>
      </div>
    </>
  );
  ...
  if (screen === "resume") return (
    <>
      ...
      <div className={`emp-app ${theme}`}>
        <div className="mesh-fixed">
          <div style={{ position: "absolute", inset: 0, background: dark ? "radial-gradient(ellipse at 50% 0%,#0d2744 0%,#070E1C 60%)" : "radial-gradient(ellipse at 50% 0%,#dbeafe 0%,#F0F6FF 60%)" }} />
        </div>
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 50, background: dark ? "rgba(7,14,28,0.9)" : "rgba(240,246,255,0.9)", backdropFilter: "blur(20px)", borderBottom: `1px solid ${dark ? "rgba(56,189,248,0.1)" : "rgba(147,197,253,0.4)"}`, padding: "12px 24px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 800, color: t, flex: 1 }}>
            📄 Your Resume
            {editedFields.size > 0 && (
              <span style={{ marginLeft: 8, fontSize: 11, padding: "2px 8px", borderRadius: 20, background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.3)", color: "#F59E0B", fontFamily: "'DM Sans',sans-serif", fontWeight: 700 }}>
                ● {editedFields.size} edits
              </span>
            )}
          </div>
          ...
          <button className="btn btn-ghost" style={{ fontSize: 13, padding: "8px 16px" }} onClick={() => { setScreen("wizard"); }}>
            🔄 Regenerate
          </button>
          <button className="btn btn-primary" style={{ fontSize: 13, padding: "8px 16px" }} onClick={() => window.print()}>
            ⬇ Download PDF
          </button>
        </div>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 780, margin: "0 auto", padding: "16px 20px 0" }}>
          <div style={{ padding: "10px 16px", borderRadius: 12, background: dark ? "rgba(56,189,248,0.08)" : "rgba(219,234,254,0.6)", border: `1px solid ${dark ? "rgba(56,189,248,0.2)" : "rgba(147,197,253,0.4)"}`, display: "flex", alignItems: "center", gap: 10, marginBottom: 16, fontSize: 13 }}>
            ...
            <span style={{ color: t, fontWeight: 600 }}>Click any text on the resume to edit it.</span>
            <span style={{ color: m }}>Edited fields show a <span style={{ color: "#F59E0B" }}>●</span> dot. Use version history above to go back.</span>
          </div>
        </div>
